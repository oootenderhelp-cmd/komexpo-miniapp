...
- [x] Справочник 16 городов-миллионников с регионами и оценкой ёмкости
- [x] Источники заявок: ключи, вход `/api/lead-intake`, статистика по источникам
- [x] Виджет для чужих сайтов: один тег script
- [x] Разбор заявок из Тильды, Яндекс Форм, лид-форм ВК и колл-трекинга
- [x] Защита внешнего входа: ключ, ограничение домена и частоты, защита от дублей
- [x] 65 тестов модуля: скоринг, SLA, маршрутизация, приём заявок, ключи, выгрузка

### Дальше

- [ ] Сверить каталог направлений и лид-магниты с прайсами партнёров
- [ ] Личный кабинет клиники-партнёра: свои заявки и статусы
- [ ] Развернуть на хостинге (нужны доступы к серверу и база) — см. `docs/deploy.md`
- [ ] Файл кампаний для Яндекс Директа под каждое направление
...
- [x] Счётчики Яндекс.Метрики и пикселя VK на `/dental`
- [ ] Запуск тестовых кампаний по семантике из `docs/dental-traffic.md`
- [x] Разрез по кампаниям: заявки, записи, приходы, доля критичных
- [ ] Цена прихода по кампаниям (нужен ввод расходов из рекламных кабинетов)
